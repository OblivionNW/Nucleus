...
public class NucleusTokenServiceImpl implements IMessageTokenService {
    ...
    @Inject
    public NucleusTokenServiceImpl(PluginContainer pluginContainer) {
        try {
            register(pluginContainer, this.tokens);
            this.tokens.getTokenNames().forEach(x -> registerPrimaryToken(x.toLowerCase(), pluginContainer, x.toLowerCase()));
